...
const AdminStudents = () => {
        ...
        try {
            ...
            if (selectedStudent) {
                ...
                try {
                    ...
                    // Only include email and documents if user is admin
                    if (user?.role === 'Admin') {
    ...
    const handleToggleVisibility = async (studentId, currentVisibility) => {
        try {
            ...
            const errorMessage = error.response?.data?.message || error.message || 'Failed to update student visibility';
            setError(errorMessage);
        }
    };
    ...
    const handleDeleteStudent = async (studentId) => {
        if (window.confirm('Are you sure you want to delete this student profile? This action cannot be undone.')) {
            try {
                ...
                const errorMessage = error.response?.data?.message || error.message || 'Failed to delete student. The student may have associated donations or other data.';
                setError(errorMessage);
            }
        }
    };
        ...
        <Box>
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 3 }}>
                ...
                <Stack direction="row" spacing={2}>
                    <Button 
                        variant="outlined" 
                        onClick={() => window.location.href = '/admin/students/by-status'}
                    >
                        View by Status
                    </Button>
                            ...
                            <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                                <CardContent sx={{ flexGrow: 1 }}>
                                    ...
                                    <Stack direction="row" spacing={1}>
                                        ...
                                        <IconButton size="small" onClick={() => navigate(`/admin/students/${student.id}`)} title="Edit Student">
                                            <Edit fontSize="small" />
                                        </IconButton>
                                        <IconButton
                                            size="small"
